...
export class ManageAdvertisementsComponent implements OnInit {
  ...
  loadAdvertisements() {
    this.isLoading = true;
    this.http.get('http://localhost:8000/api/advertisements/').subscribe({
      next: (data: any) => {
  ...
  createAdvertisement() {
    ...
    this.isLoading = true;
    this.http.post('http://localhost:8000/api/advertisements/', formData).subscribe({
      next: () => {
        this.loadAdvertisements();
        this.resetForm();
        this.isLoading = false;
  ...
  updateAdvertisement() {
    ...
    this.isLoading = true;
    this.http.put(`http://localhost:8000/api/advertisements/${this.selectedAdvertisement.id}/`, formData).subscribe({
      next: () => {
        this.loadAdvertisements();
        this.cancelEdit();
        this.isLoading = false;
  ...
  deleteAdvertisement(id: number) {
    if (confirm('Are you sure you want to delete this advertisement?')) {
      this.isLoading = true;
      this.http.delete(`http://localhost:8000/api/advertisements/${id}/`).subscribe({
        next: () => {
          this.loadAdvertisements();
          this.isLoading = false;
        },
